...
function test_configuration {
    ...
    if [ -f /usr/sbin/validator ]; then 
        if [ -f "${user_config}"/opt-in/USE_SOURCES_DIRECTLY ]; then
            /usr/sbin/validator full -relaxed "${user_config}"
            if [ $? -ne 0 ]; then 
                echo " "
    ...
    then
        ...
        echo " "
        httpd -V -S -d ${target} -f ${target}/conf/httpd.conf
        echo " "
        echo httpd  -d ${target} -f ${target}/conf/httpd.conf -DENVIRONMENT_"${DEFINE_ENV}" -t ${DUMP_CONFIG_INFO}
        echo " "
        echo " "
    fi
    ...
    APACHE_PREFIX=${target} httpd -d ${target} -f ${target}/conf/httpd.conf -d ${target} -DENVIRONMENT_"${DEFINE_ENV}" -t ${DUMP_CONFIG_INFO}
    if [ $? -ne 0 ]
    then 
        echo " "
        echo "ERROR $(date) Configuration invalid, please fix and retry, "
